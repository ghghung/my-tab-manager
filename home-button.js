(() => {
    // --- BẮT ĐẦU PHẦN TẠO BỘ LỌC SVG ---

    // 1. Tạo một container SVG ẩn
    const svgContainer = document.createElementNS('http://www.w3.org/2000/svg', 'svg');
    svgContainer.setAttribute('style', 'position:absolute; width:0; height:0');

    // 2. Định nghĩa bộ lọc bên trong SVG
    const filterId = 'frosted'; // Đặt tên cho bộ lọc
    ...
    `;
    // 3. Tiêm SVG vào đầu trang body
    document.body.prepend(svgContainer);

    // --- KẾT THÚC PHẦN TẠO BỘ LỌC SVG ---

    // --- BẮT ĐẦU PHẦN LOGIC NÚT HOME ---

    // Giá trị mặc định phòng trường hợp storage chưa có
    const DEFAULTS = {
        enabled: true,
        emoji: '✈️',
        size: 60,
    ...
    let currentEmoji = DEFAULTS.emoji;

    // Tạo nút home và icon emoji bên trong
    const button = document.createElement('div');
    button.id = 'virtual-home-button';
    button.className = 'glass'; // Class để áp dụng hiệu ứng kính
    
    const icon = document.createElement('span'); // Dùng span cho emoji
    button.appendChild(icon);
    
    document.body.appendChild(button);

    // Hàm áp dụng tất cả cài đặt (kích thước, emoji, độ mờ, bật/tắt)
    const applySettings = (settings) => {
        ...
        if (config.enabled) {
            ...
            icon.style.fontSize = `${config.size * 0.5}px`;
        } else {
            button.style.display = 'none';
        }
    };

    // Hàm áp dụng vị trí (từ tỷ lệ % sang px)
    const applyPosition = (pos) => {
        ...
        if (pos.left !== undefined) button.style.left = `${pos.left * windowWidth}px`;
        if (pos.right !== undefined) button.style.right = `${pos.right * windowWidth}px`;
        if (pos.top !== undefined) button.style.top = `${pos.top * windowHeight}px`;
        if (pos.bottom !== undefined) button.style.bottom = `${pos.bottom * windowHeight}px`;
    };

    // Lắng nghe các thay đổi từ storage (bao gồm cả cài đặt và vị trí)
    chrome.storage.onChanged.addListener((changes, namespace) => {
        if (namespace === 'local') {
            if (changes.settings) {
                console.log('Phát hiện cài đặt nút home mới. Đang cập nhật...');
                applySettings(changes.settings.newValue.virtualHomeButton);
            }
            if (changes.virtualHomeButtonPosition) {
                console.log('Phát hiện vị trí nút home mới từ tab khác. Đang cập nhật...');
                applyPosition(changes.virtualHomeButtonPosition.newValue);
            }
        }
    });

    // Tải cài đặt và vị trí ban đầu khi trang được tải
    chrome.storage.local.get(['settings', 'virtualHomeButtonPosition'], (result) => {
        applySettings(result.settings ? result.settings.virtualHomeButton : DEFAULTS);
        if (result.virtualHomeButtonPosition) {
            applyPosition(result.virtualHomeButtonPosition);
        }
    });

    // Logic Kéo - Thả - Click
    // --- BẮT ĐẦU LOGIC CHO RADIAL MENU ---
    ...
    const closeRadialMenu = () => {
        ...
        const overlay = document.getElementById('v-home-overlay');
        const menu = document.getElementById('v-home-radial-menu');
    ...
    const showRadialMenu = (tabs) => {
        if (isMenuOpen) return;
        isMenuOpen = true;

        // 1. Tạo lớp phủ mờ
        const overlay = document.createElement('div');
        overlay.id = 'v-home-overlay';
        document.body.appendChild(overlay);
        // Làm cho nó hiện ra mượt mà
        setTimeout(() => overlay.style.opacity = '1', 10);
        
        // 2. Tạo container cho các bong bóng
        const menuContainer = document.createElement('div');
        menuContainer.id = 'v-home-radial-menu';
        
        // Lấy vị trí của nút home để đặt menu vào trung tâm
        const buttonRect = button.getBoundingClientRect();
        const centerX = buttonRect.left + buttonRect.width / 2;
        const centerY = buttonRect.top + buttonRect.height / 2;
        menuContainer.style.left = `${centerX}px`;
        menuContainer.style.top = `${centerY}px`;

        // 3. Tạo và định vị các bong bóng tab
        const RADIUS = 90; // Khoảng cách từ tâm (px)
        const MAX_TABS = 10;
        const tabsToShow = tabs.slice(0, MAX_TABS);
        const angleStep = (2 * Math.PI) / tabsToShow.length;

        tabsToShow.forEach((tab, i) => {
            const angle = i * angleStep - (Math.PI / 2); // Bắt đầu từ đỉnh

            // Tính toán vị trí x, y trên một đường tròn
            const x = RADIUS * Math.cos(angle);
            const y = RADIUS * Math.sin(angle);

            const bubble = document.createElement('div');
            bubble.className = 'v-home-tab-bubble';
            bubble.style.setProperty('--delay-index', i); // Cho animation nếu cần
            bubble.style.top = `${y}px`;
            bubble.style.left = `${x}px`;
            
            // THAY THẾ ĐOẠN TẠO IMG CŨ BẰNG ĐOẠN MỚI NÀY
            // --- BẮT ĐẦU LOGIC HIỂN THỊ ICON/EMOJI ---
            let iconElement;

            // Nếu có favIconUrl, tạo một thẻ IMG
            if (tab.favIconUrl) {
                iconElement = document.createElement('img');
                iconElement.src = tab.favIconUrl;
                // Vẫn giữ fallback cuối cùng nếu URL của Google API bị lỗi
                iconElement.onerror = () => { iconElement.src = 'icons/icon16.png'; };
            } 
            // Nếu không, tạo một thẻ SPAN cho emoji
            else {
                iconElement = document.createElement('span');
                iconElement.className = 'v-home-tab-emoji'; // Gán class để CSS
                iconElement.textContent = currentEmoji;
            }

            bubble.appendChild(iconElement);
            // --- KẾT THÚC LOGIC HIỂN THỊ ICON/EMOJI ---
            menuContainer.appendChild(bubble);

            // Thêm sự kiện click để chuyển tab
        ...
        // 4. Thêm sự kiện để đóng menu
        overlay.addEventListener('click', closeRadialMenu); // Click ra ngoài
        // Khi chuột rời khỏi toàn bộ vùng menu...
        menuContainer.addEventListener('mouseleave', () => {
            // ...bắt đầu một bộ đếm thời gian để đóng menu sau 250ms
            closeMenuTimeout = setTimeout(closeRadialMenu, 350);
        });

        // Khi chuột quay trở lại vùng menu...
        menuContainer.addEventListener('mouseenter', () => {
            // ...hủy ngay bộ đếm thời gian đang chờ để đóng.
            clearTimeout(closeMenuTimeout);
        });
    };

    // Lắng nghe sự kiện click chuột phải trên nút home
    button.addEventListener('contextmenu', (e) => {
        e.preventDefault(); // Ngăn menu chuột phải mặc định của trình duyệt
        if (isMenuOpen) {
            closeRadialMenu();
        } else {
            // Yêu cầu background script cung cấp danh sách tab
    ...
    // --- KẾT THÚC LOGIC CHO RADIAL MENU ---
    ...
    window.addEventListener('mousemove', (e) => {
        ...
        if (newX < 0) newX = 0;
        if (newY < 0) newY = 0;
        if (newX + button.offsetWidth > screenWidth) newX = screenWidth - button.offsetWidth;
        if (newY + button.offsetHeight > screenHeight) newY = screenHeight - button.offsetHeight;
    ...
    window.addEventListener('mouseup', () => {
        ...
        if (isDragging) {
            ...
            if (rect.left < (windowWidth / 2)) {
                positionToSave.left = rect.left / windowWidth;
                delete positionToSave.right;
            } else {
                positionToSave.right = (windowWidth - rect.right) / windowWidth;
                delete positionToSave.left;
            }
            if (rect.top < (windowHeight / 2)) {
                positionToSave.top = rect.top / windowHeight;
                delete positionToSave.bottom;
            } else {
                positionToSave.bottom = (windowHeight - rect.bottom) / windowHeight;
                delete positionToSave.top;
            }
            chrome.storage.local.set({ virtualHomeButtonPosition: positionToSave });
        }
    });
    ...
    // Lắng nghe lệnh từ background script để hiện/ẩn nút
    chrome.runtime.onMessage.addListener((request, sender, sendResponse) => {
        if (request.action === 'showHomeButton' || request.action === 'hideHomeButton') {
            chrome.storage.local.get('settings', (result) => {
                const isEnabled = result.settings?.virtualHomeButton?.enabled ?? DEFAULTS.enabled;
                if (isEnabled && request.action === 'showHomeButton') {
                    button.style.display = 'grid';
                } else {
                    button.style.display = 'none';
                }
